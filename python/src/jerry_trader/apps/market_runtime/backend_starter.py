...
from jerry_trader.shared.utils.config_builder import (
    DEFAULT_CONFIG_PATH,
...
class JerryTraderBackendStarter:
    ...
    def _init_services(self):
        ...
        if "JerryTraderBFF" in self.roles:
            from jerry_trader.apps.market_runtime.interfaces.bff import JerryTraderBFF
        ...
        if "SnapshotProcessor" in self.roles:
            from jerry_trader.packages.market_snapshot.processor import (
                SnapshotProcessor,
            )
        ...
        if "StateEngine" in self.roles:
            from jerry_trader.packages.analytics.domain.state_engine import StateEngine
        ...
        if "StaticDataWorker" in self.roles:
            from jerry_trader.packages.market_data.application.static_data_worker import (
                StaticDataWorker,
            )
        ...
        if "NewsWorker" in self.roles:
            from jerry_trader.packages.news.news_worker import NewsWorker
        ...
        if "NewsProcessor" in self.roles:
            from jerry_trader.packages.news.application.processor import NewsProcessor
        ...
        if "Collector" in self.roles:
            from jerry_trader.apps.snapshot_runtime.collector import (
        ...
        if "Replayer" in self.roles:
            from jerry_trader.apps.snapshot_runtime.replayer import (
        ...
        if "ChartBFF" in self.roles:
            from jerry_trader.apps.market_runtime.interfaces.chart_bff import ChartBFF
        ...
        if "FactorEngine" in self.roles:
            from jerry_trader.packages.analytics.domain.engine import FactorManager
        ...
        if "BarsBuilder" in self.roles:
            from jerry_trader.packages.bar_builder.bars_builder_service import (
                BarsBuilderService,
            )
        ...
        if "AgentBFF" in self.roles:
            from jerry_trader.apps.market_runtime.interfaces.news_bff import AgentBFF
