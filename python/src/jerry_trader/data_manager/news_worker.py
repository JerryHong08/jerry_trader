...
from jerry_trader.data_supply.static_data_supply.news_fetch import (
    API_NewsFetchers,
    MoomooStockResolver,
    NewsPersistence,
)
from jerry_trader.utils.async_helpers import AsyncRateLimiter, RetryConfig
            ...
            for attempt in range(RetryConfig.MAX_RETRIES + 1):
                try:
                    ...
                    if attempt < RetryConfig.MAX_RETRIES:
                        ...
                        logger.error(
                            f"Moomoo fetch failed for {symbol} after retries: {e}"
                        )
