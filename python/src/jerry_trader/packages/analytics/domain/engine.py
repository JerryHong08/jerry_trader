...
from jerry_trader._rust import price_accel, z_score
from jerry_trader.packages.market_data.feeds.unified_tick_manager import (
    UnifiedTickManager,
)
from jerry_trader.packages.market_data.infra.bootstrap.polygon_fetcher import (
    fetch_polygon_trades,
)
from jerry_trader.shared.ids.redis_keys import factor_tasks_stream
from jerry_trader.shared.time.timezone import ms_to_et_short
...
class FactorManager:
    ...
    def __init__(
        ...
        # Warn early if critical config is missing
        if not self.influx_org:
            logger.warning(
                "⚠️ InfluxDB org is empty — writes will fail. "
                "Add 'org' to your influxdb config in config.yaml"
            )
        if not self.influx_bucket:
            logger.warning("⚠️ InfluxDB bucket is empty — writes will fail.")
        if not self.influx_token:
            logger.warning(
                f"⚠️ InfluxDB token is empty (env: {influx_token_env}) — writes will fail."
            )
        logger.info(
            f"InfluxDB config: url={self.influx_url}, "
            f"org={self.influx_org!r}, bucket={self.influx_bucket!r}, "
            f"token={'***' if self.influx_token else 'MISSING'}"
        )
        ...
        def _influx_write_success(conf, data):
            logger.debug(
                f"InfluxDB async write success: {len(data) if isinstance(data, list) else 1} records"
            )

        def _influx_write_error(conf, data, exc):
            logger.error(f"InfluxDB async write FAILED: {exc}")

        def _influx_write_retry(conf, data, exc):
            logger.warning(f"InfluxDB async write retry: {exc}")
        ...
        self.write_api = write_client.write_api(
            ...
            success_callback=_influx_write_success,
            error_callback=_influx_write_error,
            retry_callback=_influx_write_retry,
        )
    ...
    def _warm_baseline(self, ctx: TickerContext) -> List[Tuple[int, float]]:
        ...
        while t <= wall_sec:
            ...
            if len(window) >= MIN_TRADES_FOR_RATE:
                ...
                accel = price_accel(recent, older)
    ...
    def _emit_bootstrap_history(
        ...
        while t <= end_ms:
            ...
            if len(window) >= MIN_TRADES_FOR_RATE:
                ...
                trade_rate_z = z_score(trade_rate, local_rate_hist)
                accel_z = z_score(accel, local_accel_hist)
    ...
    def _compute_ticker(self, ctx: TickerContext) -> None:
        ...
        trade_rate_z = z_score(trade_rate, ctx.trade_rate_history)
        accel_z = z_score(accel, ctx.accel_history)
...
if __name__ == "__main__":
    ...
    Usage:
        python -m jerry_trader.core.factors.engine
        python -m jerry_trader.core.factors.engine --replay-date 20240115
        python -m jerry_trader.core.factors.engine --manager-type theta
        python -m jerry_trader.core.factors.engine --replay-date 20240115 --manager-type replayer
