...
Usage:
    ...
    python -m jerry_trader --machine wsl2

    # Override defaults
    python -m jerry_trader --machine wsl2 --defaults.replay_date 20260115

    # Override nested config (llm model settings)
    python -m jerry_trader --machine mibuntu --llm.models.deepseek.thinking_mode true
...
class JerryTraderBackendStarter:
    ...
    def _init_services(self):
        ...
        if "JerryTraderBFF" in self.roles:
            from jerry_trader.apps.market_bff.server import JerryTraderBFF
        ...
        if "StateEngine" in self.roles:
            from jerry_trader.services.strategy.state_engine import StateEngine
        ...
        if "StaticDataWorker" in self.roles:
            from jerry_trader.services.market_data.static.static_data_worker import (
                StaticDataWorker,
            )
        ...
        if "NewsWorker" in self.roles:
            from jerry_trader.services.news.news_worker import NewsWorker
        ...
        if "NewsProcessor" in self.roles:
            from jerry_trader.services.news.processor import NewsProcessor
        ...
        if "Collector" in self.roles:
            from jerry_trader.apps.snapshot_app.collector import MarketsnapshotCollector
