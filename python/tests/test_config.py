...
Covers:
    - Loading existing prompt files from llmContext/
...
from jerry_trader.config import load_prompt
...
class TestLoadPrompt:
    ...
    def test_load_existing_prompt(self):
        """Should load the news_processor_prompt_v2.txt successfully."""
        text = load_prompt("news_processor_prompt_v2.txt")
        assert isinstance(text, str)
        assert len(text) > 0

    def test_load_system_prompt(self):
        """Should load the news_processor_system_prompt.txt successfully."""
        text = load_prompt("news_processor_system_prompt.txt")
        assert isinstance(text, str)
        assert len(text) > 0
    ...
    def test_missing_file_raises(self):
        ...
        with pytest.raises(FileNotFoundError, match="Prompt file not found"):
            load_prompt("this_does_not_exist.txt")
