...
pub trait Factor: Send + Sync {
    ...
    /// Factor type: bar, trade, or quote.
    ...
    /// Return the current (last computed) factor value.
    /// Returns None if warmup incomplete or no data processed.
    fn get_value(&self) -> Option<f64> {
        None // Default: no value (override in concrete factors)
    }
}
...
/// Trade-based factors (TradeRate, LargeTradeRatio, etc.)
///
/// Ingests trades continuously in live mode, computes on demand.
/// For backtest, uses batch computation from historical trades.
pub trait TradeFactor: Factor {
    ...
    /// Incremental update: process a single trade.
    ///
...
pub trait QuoteFactor: Factor {
    ...
    /// at 1s compute intervals. Returns (ts_ms, value) pairs —
    /// same shape as live-mode 1s compute cycle.
