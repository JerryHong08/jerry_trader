# OrderManagement（IB 下单器）功能模块文档

> 目标：说明 `src/OrderManagement` 当前阶段的架构与功能实现，并给出未来“数据库持久化扩展”和“前端连接/联调”的演进建议。

## 1. 模块定位与边界

`OrderManagement` 是交易执行与账户/持仓查询模块，主要职责：
...
文件：`src/OrderManagement/api/server.py`
...
文件：`src/OrderManagement/api/routes_orders.py` + `src/OrderManagement/services/order_service.py`
...
文件：`src/OrderManagement/services/portfolio_service.py` + `src/OrderManagement/adapter/ib_gateway.py` + `src/OrderManagement/adapter/ibkr_wrapper.py`
...
1. 启动 OrderManagement API

- 方式 A（项目自带入口）：`poetry run python -m OrderManagement.main`
- 方式 B（uvicorn）：`poetry run uvicorn OrderManagement.main:app --host 0.0.0.0 --port 8888`
...
前端类型定义已整理在：`frontend/src/types/OrderManagement.ts`（`WsMessage`、`WsEventMessage`、各事件 payload）。

消费示例（按 `type` / `event_name` 分发）：

```ts
import type { WsMessage } from '../types/OrderManagement';
...
- `DATABASE_URL`：启用 PostgreSQL 持久化（可选）
  - 示例：`postgresql+psycopg://postgres:postgres@127.0.0.1:5432/OrderManagement`
